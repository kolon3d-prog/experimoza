...
def _build_axes_and_directions(
    ...
    if axis_count == 1:
        if direction_basis == DIEFF_CARTESIAN:
            x, _ = angle_deg_to_cartesian(direction_hundredths_deg / 100)
            directions = (LONG * 1)(x or 1)
        else:
...
def _build_unrotated_condition_axes_and_directions(
    axes_offsets: Sequence[int],
    direction_basis: int,
) -> tuple[int, object, object]:
    axis_count = len(axes_offsets)
    ...
    axes = (DWORD * axis_count)(*axes_offsets)
    directions = (LONG * axis_count)()

    if direction_basis == DIEFF_CARTESIAN:
        directions[0] = 1
    elif direction_basis == DIEFF_POLAR:
        directions[0] = 9000
    elif direction_basis == DIEFF_SPHERICAL:
        directions[0] = 0
    else:
        raise ValueError(f"Unsupported direction basis: {direction_basis}")
...
def _build_condition_description(
    *,
    ...
    axis_count, axes, directions = _build_unrotated_condition_axes_and_directions(
        axes_offsets,
        direction_basis,
    )
...
def create_condition_effect(
    ...
    *,
    ...
    offset: int = 0,
    direction_hundredths_deg: int = 0,
    direction_basis: int = DIEFF_CARTESIAN,
